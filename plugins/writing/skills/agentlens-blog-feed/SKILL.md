...
1. **Resolve `MEMORY_PATH`** (ask the user, or use the default) and read it. Expected shape: `{ "processed_ids": ["<id>", ...], "last_generated_at": <ms> }`. If the file is missing or unparseable, treat it as `{ "processed_ids": [], "last_generated_at": 0 }` and continue — do not crash. `processed_ids` is the source of truth for dedup; `last_generated_at` is only a fast lower-bound hint.
