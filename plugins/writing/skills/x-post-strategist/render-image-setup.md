...
2. Verify rendering works by rendering a bundled example template:
   ...
   ```sh
   node render-image.js ../assets/quote-card.html /tmp/quote-card.png --width=1080 --height=1080
   ```
...
| Use | Width × Height |
| --- | --- |
| Square card (quote / data, X-friendly) | `1080 × 1080` |
| Landscape (X inline, 16:9) | `1600 × 900` |
...
1. Author an HTML file (inline CSS, no external network assets — embed fonts/colors directly). Start from `assets/quote-card.html` or `assets/data-card.html`.
2. Run the script with the size for the target slot.
